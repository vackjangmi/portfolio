...
test("server-renders the short URL creator", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>sooyeol\.link — 기억하기 쉬운 짧은 주소<\/title>/i);
  assert.match(html, /긴 주소는 짧게/);
  assert.match(html, /기억은 오래/);
  assert.match(html, /sooyeol\.com\//);
  assert.match(html, /짧은 주소 만들기/);
  assert.doesNotMatch(html, /codex-preview|react-loading-skeleton|Starter Project/i);
});

test("keeps persistence, redirects, and initial links wired", async () => {
  const [page, redirect, linksApi, storage, hosting] = await Promise.all([
